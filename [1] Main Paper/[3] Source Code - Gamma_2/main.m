...
%% Performance with Varying gamma_2
possible_gamma = 0:0.01:0.1;
len = length(possible_gamma);
err_MKF = zeros(1,len);
for i = 1:len
    gamma = possible_gamma(i);   % 0.025
    gamma_1 = 1 - gamma;
    gamma_2 = 1 + gamma;
    xhat_MKF = MKF(sys, y, x_0, V_0, gamma_1, gamma_2);
    err_MKF(i) = sqrt(mean(sum((x - xhat_MKF).^2,1)));
end
figure;
plot(1+possible_gamma, err_MKF, 'r', 'LineWidth', 2.5);

set(gca, 'FontSize', 18);
ylabel('RMSE','FontSize', 20, 'Interpreter', 'latex');
xlabel('$\gamma_2$','FontSize', 20, 'Interpreter', 'latex');